...
![Bash 4+](https://img.shields.io/badge/bash-%3E%3D4.0-4EAA25?logo=gnubash&logoColor=white) ![Linux Mint 22.3 Cinnamon](https://img.shields.io/badge/Linux%20Mint-22.3%20Cinnamon-87CF3E?logo=linuxmint&logoColor=white) [![Licencia GPLv3](https://img.shields.io/badge/licencia-GPLv3-blue)](LICENSE.txt)

---

> 🇪🇸 Esta es la documentación en español. La versión principal está en inglés: [🇬🇧 English README](README.md).
...
| `./script/mint-doctor.sh --lang es` | Fuerza la interfaz en español |
| `./script/mint-doctor.sh --lang en` | Fuerza la interfaz en inglés |
...
Mint-Doctor incluye la interfaz completa en español e inglés. El README principal está en inglés y esta versión queda como documentación secundaria en español.

Por defecto, el script sigue el locale efectivo (`LC_ALL`, después `LC_MESSAGES` y después `LANG`). Si es español, la interfaz se muestra en español; en cualquier otro caso usa inglés. En GNU/Linux también respeta `LANGUAGE` cuando indica explícitamente inglés o español, salvo bajo los locales `C`/`POSIX`.

También puedes elegir el idioma de forma explícita, sin tocar el script:
...
El parámetro `--lang` tiene prioridad cuando se usa junto con la variable de entorno. Si se indica un idioma no soportado, Mint-Doctor usa inglés por defecto.

## Pruebas

El proyecto incluye una batería de regresión para sintaxis, detección del locale, los dos idiomas de la interfaz, traducciones dinámicas, el parseo de argumentos, las preguntas sí/no y una pasada completa con `--dry-run --auto`. Las ejecuciones simuladas en inglés y español comparan además los comandos que se ejecutarían, para que cambiar el idioma no pueda modificar silenciosamente una reparación.

Puedes lanzarla desde la raíz del proyecto:

```bash
./tests/test.sh
```

La batería usa Bash, Python 3 y las herramientas estándar de Linux que suele tener una instalación normal de Mint. Nunca aplica una reparación real: la prueba de extremo a extremo usa siempre `--dry-run`.
