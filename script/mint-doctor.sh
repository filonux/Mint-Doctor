#!/usr/bin/env bash
# MENU: Mint-Doctor
# DESCRIPTION: Autoreparador para Linux Mint 22.3 Cinnamon
# TERMINAL: true
#
# Copyright (C) 2026 Filonux
#
# Licencia:
#   Mint-Doctor es software libre distribuido bajo los términos de la
...
# ╔══════════════════════════════════════════════════════════════════╗
# ║  MINT-DOCTOR                                                      ║
# ║  Autoreparador para Linux Mint 22.3 "Zena"                        ║
# ╠══════════════════════════════════════════════════════════════════╣
# ║  Detecta, explica y soluciona (con tu permiso) los problemas más  ║
# ║  habituales de un sistema Linux Mint con Cinnamon.                ║
# ║  Autor: Filonux                                                   ║
# ╚══════════════════════════════════════════════════════════════════╝
#
# Nota de versión: Mint 22.3 "Zena" trae Cinnamon 6.6.x. El banner detecta la
# versión de Cinnamon en tiempo de ejecución con "cinnamon --version" para no
# quedar desfasado en el próximo punto de versión.
#
# Uso:
#   ./mint-doctor.sh            Modo interactivo (menú)
#   ./mint-doctor.sh --dry-run  Arranca en modo simulación (no aplica cambios)
#   ./mint-doctor.sh --auto     Escaneo completo automático sin menú
#
# Excepciones a --auto:
#   - Importar una clave GPG nueva (módulo 3) siempre pide confirmación
#     humana explícita, repositorio por repositorio: implica empezar a
#     confiar en paquetes firmados por ese origen. Sin terminal para
#     preguntar (p. ej. cron), esas claves se omiten en vez de aceptarse.
#   - Quitar gstreamer1.0-vaapi (módulo 8) solo se ofrece en el menú
#     interactivo, nunca en --auto: solo tiene sentido si de verdad notas el
#     problema de renderizado que soluciona.
#
# Historial de versiones: ver CHANGELOG.md en el repositorio.
#
# No ejecutar como root. El script pedirá tu contraseña con sudo
# únicamente cuando una acción concreta la necesite.
...
# Crea LOG_DIR/LOG_FILE y purga sesiones antiguas. Se llama desde main() tras
# parse_args() y check_not_root(), para que ni "-h"/"-v" ni un intento de
# ejecución como root dejen archivo en disco.
init_logging() {
    ...
    if ! { touch "$LOG_FILE" 2>/dev/null && chmod 600 "$LOG_FILE" 2>/dev/null; }; then
        ...
        echo "Aviso: no se ha podido crear el registro en ${LOG_DIR}; esta sesión continúa sin guardar registro." >&2
        LOG_FILE=/dev/null
        return
    fi
...
print_banner() {
    ...
    draw_box \
        "" \
        "   MINT-DOCTOR  ·  Autoreparador v${SCRIPT_VERSION}" \
        "   ${mint_label}  ·  Cinnamon ${cinnamon_ver}" \
        ""
    if [[ $DRY_RUN -eq 1 ]]; then
        echo -e "  ${C_YELLOW}${C_BOLD}⚠ MODO SIMULACIÓN ACTIVADO — no se aplicará ningún cambio${C_RESET}"
    fi
    if [[ $MINT_VERSION_OK -eq 0 ]]; then
        echo -e "  ${C_YELLOW}${C_BOLD}⚠ ${MINT_VERSION_WARNING}${C_RESET}"
    fi
    echo
}
...
msg_ok()   { echo -e "  ${ICON_OK} $1"; log_line "OK" "$1"; }
msg_err()  { echo -e "  ${ICON_ERR} $1"; log_line "ERROR" "$1"; }
msg_warn() { echo -e "  ${ICON_WARN} $1"; log_line "AVISO" "$1"; }
msg_info() { echo -e "  ${ICON_INFO} $1"; log_line "INFO" "$1"; }
...
pause() {
    ...
    echo
    read -rp "$(echo -e "${C_DIM}Pulsa Enter para continuar...${C_RESET}")" _
}
...
_ask_yes_no_loop() {
    local prompt="$1" ans
    while true; do
        read -rp "$(echo -e "  ${C_BOLD}${prompt}${C_RESET} [s/N]: ")" ans || return 1
        case "$ans" in
...
run_step() {
    local desc="$1"; shift
    log_line "ACCION" "$desc :: $*"
    if [[ $DRY_RUN -eq 1 ]]; then
        echo -e "  ${C_YELLOW}[SIMULACIÓN]${C_RESET} Se ejecutaría: ${C_DIM}$*${C_RESET}"
        return 0
    fi
    echo -e "  ${ICON_WAIT} Aplicando: ${desc}..."
    local log_before
    ...
    if "$@" >>"$LOG_FILE" 2>&1; then
        ...
        msg_err "Falló: ${desc}"
        if [[ -n "$tail_out" ]]; then
            echo -e "  ${C_DIM}Últimas líneas de la salida:${C_RESET}"
            indent_block "$tail_out"
        fi
        echo -e "  ${C_DIM}Registro completo: ${LOG_FILE}${C_RESET}"
        return 1
    fi
}
...
get_cinnamon_version() {
    if [[ -z "$CINNAMON_VERSION_CACHE" ]]; then
        ...
        [[ -z "$CINNAMON_VERSION_CACHE" ]] && CINNAMON_VERSION_CACHE="no detectado"
    fi
...
quick_status() {
    ...
    disk_line=$(df -h / 2>/dev/null | awk 'NR==2{print $5" usado ("$4" libres)"}')
    disk_pct=$(df / 2>/dev/null | awk 'NR==2{gsub("%","",$5); print $5}')
    ram_line=$(free -h 2>/dev/null | awk 'NR==2{print $3" usados de "$2}')

    if dpkg --audit 2>/dev/null | grep -q .; then
        apt_state="${ICON_WARN} paquetes a medio instalar"
    else
        apt_state="${ICON_OK} sin problemas evidentes"
    fi

    if systemctl is-active --quiet NetworkManager 2>/dev/null; then
        net_state="${ICON_OK} activa"
    else
        net_state="${ICON_WARN} NetworkManager inactivo"
    fi
    ...
    if [[ "$upd_count" -gt 0 ]] 2>/dev/null; then
        upd_line="${ICON_WARN} $upd_count paquete(s) esperando actualizarse"
    else
        upd_line="${ICON_OK} el sistema está al día"
    fi

    echo -e "${C_BOLD}Estado rápido del sistema${C_RESET}"
    hr
    printf "  %-22s %s\n" "Disco (/):" "${disk_line:-desconocido}"
    printf "  %-22s %s\n" "Memoria RAM:" "${ram_line:-desconocido}"
    printf "  %-22s %b\n" "Paquetes / APT:" "$apt_state"
    printf "  %-22s %b\n" "Actualizaciones:" "$upd_line"
    printf "  %-22s %b\n" "Red:" "$net_state"
    if [[ -n "$disk_pct" ]] && [[ "$disk_pct" -ge 90 ]] 2>/dev/null; then
        echo -e "  ${ICON_WARN} ${C_YELLOW}El disco está casi lleno, revisa la opción 4 (limpieza de caché).${C_RESET}"
    fi
    echo
}
...
mod_info() {
    ...
    [[ -z "$mint_ver" ]] && mint_ver="desconocida"
    ...
    session_type="${XDG_SESSION_TYPE:-desconocida}"

    printf "  %s %s\n" "$(pad_right "Distribución:" 22)" "${mint_ver:-desconocida}"
    printf "  %-22s %s\n" "Kernel:" "$kernel"
    printf "  %-22s %s\n" "Cinnamon:" "$cinnamon_ver"
    printf "  %-22s %s\n" "Encendido desde:" "${uptime_str:-desconocido}"
    printf "  %s %s\n" "$(pad_right "Sesión gráfica:" 22)" "$session_type"
    ...
    # caracteres, y "Tamaño" con su "ñ" descuadraría la columna siguiente.
    ...
    echo
    printf "  %s %s %s %s %s\n" "$(pad_right "" 22)" "$(pad_right "Tamaño" 8)" "$(pad_right "Usado" 8)" "$(pad_right "Libre" 8)" "Uso%"
    printf "  %s %s %s %s %s\n" "$(pad_right "Disco /:" 22)" "$(pad_right "${d_size:-?}" 8)" "$(pad_right "${d_used:-?}" 8)" "$(pad_right "${d_free:-?}" 8)" "${d_pct:-?}"
    printf "  %s %s %s %s\n" "$(pad_right "" 22)" "$(pad_right "Total" 8)" "$(pad_right "Usado" 8)" "Libre"
    printf "  %s %s %s %s\n" "$(pad_right "Memoria RAM:" 22)" "$(pad_right "${m_total:-?}" 8)" "$(pad_right "${m_used:-?}" 8)" "${m_free:-?}"
    echo
...
mod_apt() {
    ...
    if ! need_cmd fuser; then
        ...
        if [[ ${#stale_locks[@]} -gt 0 ]]; then
            ...
            printf "      %s\n" "${stale_locks[@]}"
            echo -e "  ${C_DIM}Esto suele ocurrir tras un apagado brusco o un 'Administrador de actualizaciones' colgado.${C_RESET}"
...
mod_repos() {
    ...
    if [[ ${#stale_repo_files[@]} -gt 0 ]]; then
        ...
        if ask_yes_no "¿Comentar (con copia de seguridad) esas líneas para dejar de usarlas?"; then
            if [[ $DRY_RUN -eq 1 ]]; then
                echo -e "  ${C_YELLOW}[SIMULACIÓN]${C_RESET} Se comentarían las líneas/bloques de: ${C_DIM}$(printf '%s ' "${stale_repo_files[@]%%:*}")${C_RESET}"
                ...
                if run_step "copia de seguridad de /etc/apt/sources.list.d" \
                    ...
                    for entry in "${stale_repo_files[@]}"; do
                        ...
                        log_line "ACCION" "comentar referencia a $cn en $file_cn"
                        echo -e "  ${ICON_WAIT} Aplicando: comentar referencia a $cn en $file_cn..."
    ...
    if [[ -n "$missing_keys" ]]; then
        ...
        for key in $missing_keys; do
            found_issue=1
            register_issue
            echo -e "      ${C_BOLD}Clave:${C_RESET} $key"
            echo -e "      ${C_BOLD}Repositorio:${C_RESET} ${key_repo[$key]:-no identificado, revisa el log}"
                    ...
                    for host in $bad_hosts; do
                        ...
                        for list_file in /etc/apt/sources.list /etc/apt/sources.list.d/*.list /etc/apt/sources.list.d/*.sources; do
                            ...
                            if grep -q "$host_re" "$list_file" 2>/dev/null; then
                                log_line "ACCION" "comentar referencia a $host en $list_file"
                                echo -e "  ${ICON_WAIT} Aplicando: comentar referencia a $host en $list_file..."
...
mod_cache() {
    ...
    if need_cmd flatpak; then
        ...
        if ask_yes_no "¿Buscar y eliminar runtimes/dependencias de Flatpak que ya no usa ninguna aplicación?"; then
            if [[ $DRY_RUN -eq 1 ]]; then
                echo -e "  ${C_YELLOW}[SIMULACIÓN]${C_RESET} Se ejecutaría: ${C_DIM}flatpak uninstall --unused --noninteractive --assumeyes${C_RESET}"
            else
                ...
                log_line "ACCION" "eliminar dependencias Flatpak sin usar"
...
mod_boot() {
    ...
    if [[ -f /boot/grub/grub.cfg ]]; then
        ...
        if ! grep -q "$current_kernel_re" /boot/grub/grub.cfg 2>/dev/null; then
            ...
            echo -e "  ${C_DIM}Suele pasar tras instalar un kernel nuevo sin regenerar GRUB después.${C_RESET}"
    ...
    if [[ -d /boot/efi/EFI/Microsoft ]] && need_cmd timedatectl; then
        ...
        if [[ "$rtc_local" == "no" ]]; then
            ...
            echo -e "  ${C_DIM}Windows espera que el RTC esté en hora local, así que Linux o Windows mostrarán"
            echo -e "  ${C_DIM}la hora equivocada justo después de arrancar el otro sistema.${C_RESET}"
        ...
        if [[ ${#untrusted[@]} -gt 0 ]]; then
            ...
            if ask_yes_no "¿Marcarlos como confiables para que funcionen con un doble clic?"; then
                if [[ $DRY_RUN -eq 1 ]]; then
                    ...
                    echo -e "  ${C_YELLOW}[SIMULACIÓN]${C_RESET} Se marcarían como confiables: ${C_DIM}${names_str}${C_RESET}"
                else
                    ...
                    if [[ $ok_count -gt 0 ]]; then
                        ...
                        ISSUES_FIXED=$((ISSUES_FIXED + 1))
                        log_line "ACCION" "marcar como confiables $ok_count accesos directos en $desktop_dir"
                    else
...
print_help() {
    cat <<EOF
${SCRIPT_NAME} v${SCRIPT_VERSION} · Autoreparador para $(get_mint_label) (Cinnamon)

Uso:
  $(basename "$0")                    Modo interactivo (menú)
  $(basename "$0") --dry-run          Simula las acciones, no aplica ningún cambio
  $(basename "$0") --auto             Analiza y repara todo automáticamente, sin menú
  $(basename "$0") --dry-run --auto   Escaneo completo simulado, sin menú
  $(basename "$0") -h, --help         Muestra esta ayuda
  $(basename "$0") -v, --version      Muestra la versión y sale

No lo ejecutes con sudo ni como root: el propio script pedirá la contraseña
únicamente cuando una acción concreta la necesite.

Copyright (C) 2026 Filonux
Licencia GPLv3; consulte el archivo LICENSE para el texto completo.
EOF
}
...
parse_args() {
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            -v|--version) echo "${SCRIPT_NAME} v${SCRIPT_VERSION} · Filonux · Licencia GPLv3"; exit 0 ;;
            *)
...
check_not_root() {
    if [[ $EUID -eq 0 ]]; then
        echo -e "${C_RED}${C_BOLD}No ejecutes Mint-Doctor directamente como root ni con sudo.${C_RESET}" >&2
        echo "El script pedirá la contraseña con sudo solo cuando una acción concreta la necesite." >&2
        exit 1
    fi
}
...
print_summary() {
    echo
    hr
    echo -e "${C_BOLD}Resumen de la sesión${C_RESET}"
    printf "  %-24s %d\n" "Problemas detectados:" "$ISSUES_TOTAL"
    printf "  %-24s %d\n" "Solucionados:" "$ISSUES_FIXED"
    printf "  %-24s %d\n" "Omitidos:" "$ISSUES_SKIPPED"
    echo
    ...
    if [[ $AUTO_MODE -eq 1 ]] && [[ $ISSUES_TOTAL -gt 0 ]] \
       && [[ -n "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]] && need_cmd notify-send; then
        local notify_msg="${ISSUES_TOTAL} problema(s) detectado(s), ${ISSUES_FIXED} solucionado(s). Registro: ${LOG_FILE}"
        # En --dry-run "solucionado(s)" siempre es 0 y puede leerse como que
        # ya se aplicaron cambios; se aclara para no inducir a error en una
        # notificación que, a diferencia del banner en pantalla, puede verse
        # sin ningún contexto adicional alrededor.
...
main_menu() {
    ...
    while true; do
        print_banner
        quick_status
        echo -e "${C_BOLD}¿Qué quieres revisar?${C_RESET}"
        hr
        echo "  1) Información del sistema"
        echo "  2) Paquetes y APT/dpkg"
        echo "  3) Repositorios de software (orígenes y claves GPG)"
        echo "  4) Espacio en disco y limpieza de caché"
        echo "  5) Arranque, GRUB y reloj del sistema"
        echo "  6) Conectividad de red"
        echo "  7) Salud del hardware (disco, batería y controladores)"
        echo "  8) Escritorio Cinnamon"
        echo "  9) Copias de seguridad (Timeshift)"
        echo " 10) Ejecutar todos los módulos"
        echo "  0) Salir"
        echo
        read -rp "$(echo -e "${C_BOLD}Elige una opción${C_RESET} [0-10]: ")" opt || break
        case "$opt" in
            1) mod_info; pause ;;
            2) mod_apt; pause ;;
            3) mod_repos; pause ;;
...
main() {
    ...
    if [[ $AUTO_MODE -eq 1 ]]; then
        ...
        main_menu
        echo
        echo -e "${C_CYAN}Hasta la próxima. ¡Que tu Mint funcione de maravilla!${C_RESET}"
    fi
...
on_interrupt() {
    echo -e "\n${C_YELLOW}Interrumpido por el usuario.${C_RESET}"
    print_summary
    exit "$((128 + $1))"
}
